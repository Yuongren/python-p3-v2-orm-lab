...
class Review:
    ...
    def employee_exists(employee_id):
        # Query the Employee table to check if the employee exists
        result = CURSOR.execute("SELECT id FROM employees WHERE id = ?", (employee_id,))
    ...
    def employee_id(self, new_employee_id):
        """Setter method to update employee_id with foreign key validation."""
        if not self.employee_exists(new_employee_id):
